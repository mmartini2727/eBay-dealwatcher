...
- **Baselines last recomputed ~6 days ago (V1.0 live verification, 2026-09-18) — alerts are currently scoring against stale percentiles.** `scripts/recompute_baselines.py` is still manual-only (no cron, no collector hook); the `baselines_age` indicator is the only thing watching this at all, and it only surfaces on the dashboard/`get_system_health` for someone to notice. This is now live evidence for actually doing the "schedule the recompute" work design.md §15's Deliberately Deferred section already pairs with `baseline_history` — not a new problem, a reason to stop deferring both.
