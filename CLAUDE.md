...
- **V0.7 complete.** Normalize engine wired to the database. Live: 1,317 listings — ok 638, rejected 473, partial 166, not_target 35, pending 5.
...
- **Live scoring verified.** 497 listings scored, all from seeds — zero computed baselines, since the best bucket sits at 11 of 12 fast candidates. Gen 1–4 ratios cluster near 1.0; Gen 5/6 run 1.05–2.97, i.e. no current deals and possibly conservative seeds.

### Open items

- **`storage_tier` removal from `bucket_key`** — next milestone. You don't act on storage, but it splits every bucket four ways against a `min_samples` you can't reach. Dropping it likely gets `2|intel-11th|16` and `1|intel-10th|16` over the line. Requires full backfill re-run; six specific seed overrides name `storage_tier` and need rewriting to RAM-only.
- **`?` buckets cost 41% of the dead pool.** RAM/storage parse failures. Three Ultra 1 listings spanning $499–$1,099 share `5|intel-ultra-1|?|?` and get one seed between them. Biggest remaining accuracy problem after storage.
...
- **Fast-sale premise still unfalsified.** No bucket has ≥5 fast and ≥5 slow candidates. Watch report section (d).
- **Gen 5/6 seeds unvalidated** against any observed fast sale. Review after the first one lands.
- **Never-swept rate flat at ~8% of daily deaths**, before and after V0.7c. Pagination doesn't explain it; cause unidentified. Do not tune `MISS_THRESHOLD` until it is. Resurrection cohorts (1082; 2836 ×7; 5670 ×3) are part of the same open question — the 5670 cohort spanned two sellers, ruling out batch relisting.
