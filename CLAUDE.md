...
└── mcp_server/
  ├── server.py (V1.0) streamable HTTP, LAN only — separate compose service/process, never imports dealwatch.main; all eight tools now built — prompt 1's get_system_health, trace_title, explain_listing, plus prompt 2's query_listings, find_deals, get_market_price, get_alert_activity, get_review_queue
  ├── item_lookup.py (V1.0) resolve_item_ids() — item_id/URL/bare-legacy-number resolution for explain_listing()
  ├── formatting.py (V1.0) money_display/time_display/age_display/parse_iso — independent of reporting/panels.py's own private display helpers, same conventions by agreement, not shared code
  └── vocabulary.py (V1.0 prompt 2a) build_bucket_vocabulary() — per-field valid-value source for generation/cpu_family/ram_tier (tiers → derive → extract → observed-in-listings, first that's fully literal wins), computed once at server.py import time; the enforcement point behind get_market_price/query_listings' input validation, not just their tool descriptions
```
...
- **Three deploy modes, not two — match the mode to what actually changed.**
  ...
  - **V1.0: two containers, one image — pick the target(s) deliberately.**
    `dealwatch` (the collector) and `dealwatch-mcp` (design.md §15) share
    one built image but run as separate containers with separate
    lifecycles.
    - MCP-only change (a tool description, `mcp_server/`'s own code):
      `docker compose up -d --build dealwatch-mcp` — rebuilds the shared
      image (both services reference the same tag), but only recreates
      the MCP container. The collector keeps running, mid-poll, untouched.
    - A shared-code change (`engine/`, `normalize/`, `storage/`,
      `reporting/` — anything either container imports): unqualified
      `docker compose up -d --build` with no service name, so both
      containers end up running the identical rebuilt image. Rebuilding
      only one half would leave them disagreeing about what the code does
      to the same database.
    - A profile edit: `docker compose restart dealwatch dealwatch-mcp` —
      **both**, not just one. Each process loads `profiles/*.yaml` once,
      at its own startup (design.md §15 D13) — restarting only the
      collector leaves the MCP server answering questions against a
      profile it disagrees with the collector about, with no error to
      signal the mismatch.
    - **A change that adds a migration deploys the collector first**
      (design.md §15 D5). `connect_readonly()` never migrates — the MCP
      server would query a column that doesn't exist yet if it started
      (or restarted) against the new code before the collector has run
      the migration at least once.
    - **A change to `compose.yaml` itself (the `image:` tag, a volume, an
      env var name) is a shared-code-class change, not a targeted one —
      even if it only touches one service's stanza.** Live-verified
      2026-09-18: adding an explicit `image:` tag to `compose.yaml`, then
      running a TARGETED `docker compose up -d --build dealwatch-mcp`
      (correct for an MCP-only code change) left `dealwatch` — the
      collector — running under its OLD, pre-tag, auto-generated image
      name for about a day. `up` only recreates the service(s) named on
      the command line; it does not re-evaluate whether an unnamed
      service's compose-file-level config (like its own `image:` line)
      changed. The fix is the unqualified `docker compose up -d --build`
      any `compose.yaml` edit already calls for. **Check with
      `docker ps -a --format '{{.Names}}\t{{.Image}}'`** — both
      containers should show the SAME image reference; if they don't,
      one of them is running stale, and no error will say so.
...
- **Every MCP tool (`dealwatch/mcp_server/server.py`) is a plain `def`, never `async def` — same shape of pin, different reason.** This process has no collector to stall (design.md §15 D2 — it never imports `dealwatch.main`), but an `async def` tool still runs ON this server's own event loop (confirmed against `mcp==2.2.0`'s source, `func_metadata.py`: a sync tool dispatches via `anyio.to_thread.run_sync`, an async one does not) and would serialize every concurrent tool call — and `/health` — behind whichever one is mid-query. Pinned by `tests/test_mcp_server.py::test_every_registered_tool_is_a_sync_function`; sabotage-verified by changing one tool to `async def` and confirming the test fails.
...
- **V0.12 complete — LAN dashboard enrichment.** Baseline progress bars,
resolved seed p25/p50 for buckets awaiting a computed baseline, and
14-day alert summary stats. Schema unchanged at version 8.

Dashboard: LAN-only, unauthenticated, port 8087 (uvicorn 8000
in-container), read-only at the driver level via connect_readonly().
The route is a sync `def` and must stay that way — the collector runs
in-process on the same event loop, and `async def` there stalls poll
and sweep for the duration of the render. Pinned by a test; the test
is deliberate, not pedantic.

baselines recompute is manual (scripts/recompute_baselines.py). The
baselines_age indicator is the only thing that reports staleness. It
caught a 6.7-day gap where 3 of 5 qualifying buckets had no baseline
and were being scored off seed values.

templates/dashboard.html has no test coverage by design. Every visual
...
- **V0.13 complete — sweep bookkeeping invariant correction + reporting-path log volume (design.md §14).** Two independent parts, no schema change (still version 8), no behavior change to the collector, `record_sighting`, or `record_sweep`. **Part A:** `reporting/status.py`'s `sweep_bookkeeping_consistent` compared the UNFILTERED `MAX(listings.last_seen)` against `last_sweep_started_at`, on the documented premise that only `record_sweep()` advances `last_seen` — false: `record_sighting()`'s insert branch writes `last_seen` once, for every brand-new listing, and at the live rate (~31 new listings/day against hourly sweeps) an ordinary poll discovering a new listing pushed the check amber on a completely healthy collector close to half the time; confirmed live (one newly-inserted listing, indicator false, no other symptom). Fixed by excluding rows that didn't exist when the sweep ran (`first_seen <= last_sweep_started_at`) before comparing `MAX(last_seen)` against the stamp; new payload field `listings_ahead_of_last_sweep` (both this and `sweep_bookkeeping_consistent` stay `None`, never `False`/`0`, across three unevaluable cases: no recorded sweep, `distinct_count = 0`, or no listing at all predates the sweep). `indicators.py`'s bookkeeping value now distinguishes "N listings ahead of last sweep" from "no listing carries the last sweep stamp" instead of a bare "mismatch" string. `listings_last_seen_max` itself is unchanged — still the honest unfiltered answer. The false "only the sweep writes `last_seen`" claim was corrected everywhere it was stated by citation, not just at the one check that broke: `reporting/status.py`, `storage/sqlite.py`'s `record_sighting()` docstring, and `docs/design.md` §4.2 (plus a new §14 dated entry) — recorded as `docs/learnings.md` L10. Four required test cases added to `tests/test_status.py`, each sabotage-verified red (dropping the `first_seen <=` clause, weakening `>` to `>=`, returning `0` instead of `None` in the no-pre-sweep-rows branch). **Part B:** the per-item negative-lifespan drop count (`engine/baselines.py`) is no longer log-only — new `CandidatePoolStats.negative_lifespan_dropped` field and public `derive_candidates_with_stats()` (still one `_derive()` implementation, per that module's own "one exclusion definition" rule); `reporting/panels.py`'s `baseline_queue()` now returns `{"queue": [...], "negative_lifespan_dropped": n}` instead of a bare list, reusing the SAME derive call it already makes for ranking rather than a second one; `dashboard.html` renders "N candidates dropped: negative lifespan (docs/learnings.md L3)" under the queue when nonzero, nothing when zero; `scripts/baseline_report.py` prints the same stat in its existing candidate-pool section. The aggregate log line itself demoted INFO→DEBUG — not a volume fix (measured at 11 lines/10min with a dashboard tab open, ~0 with none, bounded by the 30s payload TTL — nowhere near log-flooding; see `docs/learnings.md` L11 for two measurement mistakes made diagnosing that number), but because the count now lives on the page and the log line has no reader left; `scripts/recompute_baselines.py`/`baseline_report.py` still set DEBUG in their own `main()`, so a maintainer running either directly still sees it. **Review caught one defect before deploy:** the drop-count line was first written inside `{% elif payload.baseline_queue.queue %}` — the same branch as the queue list — so it only ever rendered alongside a non-empty queue, and silently disappeared the moment every bucket reaches a computed baseline (`queue: []`), which is both the state this project is working toward and the state where a nonzero drop count is most worth seeing. `dashboard.html` has no test coverage by design, so nothing automated exercised an empty-queue-plus-nonzero-count fixture. Fixed by making the line a sibling of the queue-or-empty-message branch rather than a child of it; recorded as `docs/learnings.md` L12. **Not run against the real LXC from this checkout** — the milestone's two live-verification queries (the pre/post-deploy `sqlite3` count against `data/dealwatch.db`, and the `docker logs` grep) both need the actual deployed container; this checkout's local `data/dealwatch.db` is empty (0 bytes), not the production database. Live verification against real data DID surface one open item not fixed by this milestone: `last_seen` can predate `first_seen` (2 rows, delta −2) — see Open items below and `docs/learnings.md` L3's correction and L13.

- **V1.0 prompt 1 — live-verified 2026-09-18 (design.md §15).** New `dealwatch/mcp_server/` package: three tools (`get_system_health`, `trace_title`, `explain_listing`), a `/health` route, and `compose.yaml`'s `dealwatch-mcp` service, all built against `mcp==2.2.0`. No change to `engine/`, `normalize/`, `storage/`, `reporting/`, `notify/`, `providers/`, `dashboard/`, `main.py`, `scripts/`, or the schema — confirmed by diff, not just intent. `server.py` never imports `dealwatch.main` (subprocess-verified); every tool is a plain `def` (sabotage-verified: one tool changed to `async def`, the sync-pin test failed, reverted); the read-only connection helper's `connect_readonly()` is what actually blocks writes, not the volume mount (sabotage-verified: swapped to `connect()`, a write that should raise `sqlite3.OperationalError` instead succeeded); host validation uses the app's real `TransportSecuritySettings`, built from `Settings.mcp_allowed_hosts` (new field, env var `MCP_ALLOWED_HOSTS` — no `env_prefix` on `Settings`, so this is NOT `DEALWATCH_MCP_ALLOWED_HOSTS` as first guessed). 34 tests, `tests/test_mcp_server.py`, full suite 589 passed. Two SDK behaviors found during the build, not in the original brief: a `StreamableHTTPSessionManager` can only `.run()` once per instance (a test needing more than one HTTP round trip must build a fresh app via the new `_build_app()` helper, not reuse the module-level `app`); `@mcp.tool()` returns the original function unchanged, so every tool is callable directly in tests without the JSON-RPC layer. One real content gap against the original tool contract, not just a rename: `get_system_health` has no persisted "last poll" timestamp to report (that lives only in the collector's own in-process `CollectorStats`, unreachable without importing `dealwatch.main`, which is forbidden) — reports `last_price_change` instead, labeled for what it actually is (rewritten again post-review: explicit that liveness claims come ONLY from `collector`, and that a long gap on `last_price_change` in a quiet market is normal, not a stall — both in the tool description and as an inline `note` on the field itself). **Review caught a real test-coverage gap:** every direct-call test proves the tool's LOGIC is right, not that the SDK can actually serialize what it returns — a tool can hand back a fine Python dict that still fails at the JSON-RPC boundary while every direct-call test stays green. `get_system_health` was under specific suspicion (it wraps `collect_status()`'s payload, built for Jinja and a CLI, not JSON) and does serialize cleanly, but that had only ever been confirmed by a throwaway manual probe, never a committed test. The end-to-end test now calls all three tools plus `explain_listing`'s not-found path over the real HTTP/JSON-RPC layer, asserting `isError: false` and that the content parses as JSON. Full accounting of every SDK fact, deviation, and the wheel-build/`docker compose config` checks that stood in for a real `docker build` (no daemon available in this checkout) is design.md §15's build addendum. **Live-verified against the real LXC 2026-09-18** — full account (the `image:`-tag deploy-mode trap above, the "connected is not called" finding now `docs/learnings.md` L14, and two open items neither caused by V1.0) is design.md §15's dated live-verification entry.

- **V1.0 prompt 2 built — not yet live-verified (design.md §15).** The remaining five tools: `query_listings`, `find_deals`, `get_market_price`, `get_alert_activity`, `get_review_queue` — all eight tools now exist. No change to `engine/`, `normalize/`, `storage/`, `reporting/`, `notify/`, `providers/`, `dashboard/`, `main.py`, `scripts/`, `Dockerfile`, `compose.yaml`, or the schema. `find_deals` measured at 14ms against a synthetic 1,200-row eligible-listing fixture (no Docker daemon and an empty local db in this checkout, same limitation as prompt 1 — the real production number is Myke's to measure per the live-verification steps) — well under the ~2s threshold, so no cache was added. `get_market_price` resolves the same computed-then-seed ladder `score_listing()` uses, but builds its spec dict directly from the tool's own `generation`/`cpu_family`/`ram_tier` arguments rather than picking a representative listing the way `reporting/panels.py`'s `baseline_queue()` has to — a different tool shape, not a fork of that function's own reasoning (design.md §15's build addendum has the full distinction, since at a glance it resembles the exact pattern `baseline_queue()`'s docstring warns against). `get_review_queue`'s missing-bucket-field detection reads `spec_json` against `profile.bucket_key` directly (the authoritative field-order source, not a guess) rather than reverse-parsing `bucket_key`'s string positions — same reasoning. 18 new tests (52 total in `tests/test_mcp_server.py`), full suite 607 passed; every required test sabotage-verified, ten via a real manual edit-run-revert pass (inline query/return-shape logic with no importable seam to monkeypatch) and the rest as committed automated sabotage tests. Full accounting — the timing measurement, every function signature reused, both contract deviations, and the ten sabotage transcripts — is design.md §15's prompt-2 build addendum.

- **V1.0 prompt 2a built — not yet live-verified (design.md §15).** The live pass found a real defect: `get_market_price` called with `generation="Gen 2"`, `cpu_family="Ryzen 5000"`, `ram_tier="16GB"` didn't error — the malformed bucket_key had no computed row, fell through to the seed layer (a legitimate branch for a different situation), and returned a confident, correctly-caveated, wrong price (`docs/learnings.md` L15: **a tool description is advice a model may drop; anything that selects a code path must be enforced in tool code**). New `dealwatch/mcp_server/vocabulary.py`: `ram_tier` enumerable from `profile.tiers`; `generation` enumerable from `profile.derive` (its own `extract` rules are template-valued, but every `derive` rule targeting it maps a `cpu_family` to a literal digit); `cpu_family` falls back to observed values in `listings.spec_json` (its `extract` rules mix literals with two capture-group templates). `get_market_price`/`query_listings` now reject an out-of-vocabulary `generation`/`cpu_family`/`ram_tier` as a structured error (case-insensitive exact match, canonical spelling returned — no fuzzy matching, sabotage-verified: a `difflib` fallback let `"Ryzen 5000"` silently resolve, reverted) instead of falling through or silently matching zero rows. Vocabulary computed once at import time — profile-sourced fields need no DB, but the observed-fallback branch is wrapped in try/except so a not-yet-writable database at container boot degrades to an empty (all-reject) vocabulary for that field rather than crashing the module import, the same `/health`-style "up means readable, not just running" posture applied to startup instead of a request. `get_market_price` also now distinguishes `no_computed_baseline` (bucket has real history, under `min_samples`) from `bucket_never_observed` (no listing has ever normalized into this exact bucket) — previously identical. `resolved_bucket_key` added to `get_market_price`, every `find_deals` row, and `explain_listing`. `find_deals`' own generation/cpu_family/ram_tier filters were deliberately NOT given validation (out of the milestone's stated scope) — they still silently zero-match a bad value. 10 new tests (62 total in `tests/test_mcp_server.py`, 617 full suite); six sabotage-verified via real manual edit-run-revert (inline validation blocks, a hardcoded docstring literal, a hardcoded stale vocabulary dict, `ever_observed = True`), one by commenting out `explain_listing`'s field assignment. Full accounting is design.md §15's prompt-2a build addendum.

- **V1.0 prompt 2b built — not yet live-verified (design.md §15).** Closes three consequences of 2a's own build (`docs/learnings.md` L15's extension): a degraded observed vocabulary was invisible (`/health` still said 200 while `cpu_family` rejected everything) and permanent (fixed at import, no restart meant no fix); a newly-observed `cpu_family` value was falsely rejected until a restart; `find_deals`' own bucket-field filters, correctly left unvalidated by 2a's stated scope, still silently zero-matched a bad value. `dealwatch/mcp_server/vocabulary.py` split into `resolve_profile_vocabulary()` (no I/O, called once at import, unchanged) and `fetch_observed_vocabularies()` (the DB read, now callable on demand); `server.py`'s new `_current_bucket_vocabulary()` is the one accessor every tool's validation and `/health` both call, TTL-rebuilding the observed half (60s, one `threading.Lock`, same shape as `dashboard_data.py`'s own cache) while profile-sourced fields stay fixed from import (sabotage-verified: routing them through the rebuild path anyway registered calls a monkeypatched `resolve_profile_vocabulary()` should never see again). **A failed or empty build is never cached** — the fix that makes the degradation self-healing rather than permanent; sabotage-verified both directions (caching an empty result kept it stuck, skipping the TTL check kept a real new value rejected). `/health` now reports a `vocabulary` block (per field: `source`, `count`, `built_at`) and a top-level `vocabulary_degraded` bool at **200, not 503** — the existing schema-version check is what still 503s on a genuinely unreadable database; a narrow, single-field, self-healing degradation most tools don't touch gets a visible flag instead of paging over the whole server (design.md §15's dated 2b addendum has the full reasoning). Rejection wording now differs by source: observed-sourced says "has not been observed in any collected listing" (proved absent from collected data, not impossible); profile-sourced keeps "unknown". `find_deals` now validates `generation`/`cpu_family`/`ram_tier` identically to the other two tools. 8 new tests (70 total in `tests/test_mcp_server.py`, 625 full suite), all eight sabotage-verified via real manual edit-run-revert. Full accounting is design.md §15's prompt-2b build addendum.

- **`docs/learnings.md` L2 shipped 2026-09-20 — `engine/baselines.py`'s candidate query is now `profile_id`-scoped (design.md §16 P6).** `_DEAD_OK_LISTINGS` gained `profile_id = ?` in its WHERE clause; `_derive()`, `derive_candidates()`, `derive_candidate_pool_stats()`, and `derive_candidates_with_stats()` all take `profile_id` as a **required keyword-only argument with no default** — no `str | None = None`, no module-level fallback. Six call sites updated, not four as earlier drafts of design.md §16 and learnings.md L2 said: `scripts/recompute_baselines.py`, `scripts/baseline_report.py` (two calls), `reporting/panels.py::baseline_queue()`, `mcp_server/server.py::get_market_price()`, and `scripts/bucket_key_dryrun.py` (two calls — `verify_against_derive_candidates()` and `derive_pre_bucket_candidates()`, which runs its own textually-derived query, `_DRY_RUN_LISTINGS`; see `docs/learnings.md` L16 for the `.replace()` coupling this exposed). The `bucket_key_dryrun.py` sites were found by a required pre-edit call-site enumeration, not by either planning document's existing list. Comments claiming the query was unscoped corrected at every site that stated it: `reporting/status.py`, `reporting/panels.py`, `engine/baselines.py`, `mcp_server/server.py`. Required test: a two-profile fixture whose dead listings collide on the same `bucket_key` string, asserting each profile's percentiles come out unmixed — sabotage-verified by a real edit-run-revert pass (defeat as `profile_id = ? OR 1 = 1`, keeping the bound parameter so binding doesn't error), predicted red/green stated before running: only the new test went red, all 27 pre-existing `test_baselines.py` calls and the full suite stayed green both before sabotage and after revert (626 passed). Schema unchanged, still version 8. **Live-verified 2026-09-20 against the real LXC, one profile:** unfiltered and filtered candidate counts both 778 — the filter is a confirmed no-op with one profile, checked directly against the database. **Coverage limit, stated as a limit:** the sabotage proves the filter is load-bearing inside `engine/baselines.py`; it does not prove any of the six call sites passes the *correct* `profile_id` — a site passing a wrong-but-valid value would leave the new test green, since the test exercises the engine function directly, not any call site's own argument. Not covered by choice; the write path (`run_recompute()` against a two-profile fixture) is where a wrong-value bug would actually surface, and it is not exercised here. Full accounting is design.md §16 P6's dated entry and `docs/learnings.md` L2/L16.

- **Nightly baseline recompute shipped 2026-09-21 (design.md §17).** `scripts/recompute-baselines.sh`, a host-side wrapper run by cron at 01:45 local — 15 minutes after the existing 01:30 snapshot job, so a bad recompute is always recoverable from a snapshot taken minutes earlier. It globs `profiles/*.yaml`, skips any profile with `enabled: false`, runs `docker exec dealwatch python scripts/recompute_baselines.py --profile <rel>` per profile (the collector's own loop is never touched — a recompute there would block poll and sweep for its duration), and pushes success/failure to an Uptime Kuma Push monitor (~26h heartbeat window). Kept as its own script rather than appended to `snapshot.sh`, since `snapshot.sh <tag>`'s manual pre-migration form must not have the side effect of also triggering a recompute. Two guards, both **sabotage-verified on the real LXC, not in tests**: (1) `store_baselines()` is DELETE-then-INSERT in one transaction, so a run that derives zero qualifying buckets wipes the profile's baselines outright rather than leaving them stale — `baselines_age` can't flag an empty table as old. The wrapper counts rows before/after and fails the run if a non-empty set becomes empty (verified with a profile copy set to `min_samples: 9999` — guard fired, `exit=1`). This is a detector, not a lock; acceptable only because the recompute is cheap/idempotent and a snapshot exists 15 minutes prior. (2) Two profile YAMLs sharing an `id:` silently clobber each other's baselines — found by accident sabotage-testing guard (1); guard (1) does not catch it since the post-run count is non-zero. The wrapper now collects every profile's `id:` before the loop starts and refuses the whole run on any duplicate. Full accounting, including a third fix (an unquoted `&` in the Kuma URL splitting into background jobs and silently losing the env var) is `docs/learnings.md` L17/L18/L19. Live-verified 2026-09-21: clean run, `baselines 8 -> 8`, 741 dead candidates considered, Kuma green.
...
- **Fast-sale premise: holding at current n, still open (updated 2026-09-20 live report).** Eleven buckets now have ≥5 fast and ≥5 slow candidates (`baseline_report.py` section (d), up from three at the prior measurement) — **all eleven hold**, spanning both vendors, five generations, and three RAM tiers, fast-side n up to 69. `3|amd-ryzen-6000|16`, the one bucket that violated at the prior measurement, reversed as n grew (fast n=16 $455.00 vs. slow n=9 $474.99 — fast now cheaper, as the premise predicts; see `docs/learnings.md` L4's dated correction). This is the strongest evidence the premise has had, not proof — n is still under 10 on the fast side in several of the eleven buckets, and every measurement so far is one marketplace over one window. Not closing this item on the strength of one report run.
- **`fast_lifespan_hours: 24` currently selects almost exactly half the candidate pool, not a tail — now measured across four thresholds (2026-09-20 live report).** 2h: 93 fast, 2 buckets qualify. 6h: 176 fast, 4 buckets qualify. 24h: 332 fast, 8 buckets qualify (332/678 = 49%). 72h: 452 fast, 10 buckets qualify. The tradeoff is visible in both directions now: 6h isolates a real tail (26% of the pool) but halves the number of buckets with enough data on both sides to even run the fast-sale-premise check above; 72h nearly doubles the fast pool over 24h but stops looking like "fast" in any meaningful sense. No threshold change decided here — this is the distribution the item was waiting on, not a recommendation.
...
- **Never-swept rate: unchanged, and the post-deploy check is still owed, not run (updated 2026-09-20 live report).** Original: 22 of 217 dead listings (10%). Current: 778 dead spec-ok listings, 701 confirmed present by at least one sweep, 77 never swept — 9.9%, statistically the same rate. Backing out the increment (roughly 561 listings died between the two measurements, about 55 of them never swept) doesn't change the picture — still ~10%. This is **not** a refutation of the V0.8d pagination-drift model or the `MISS_THRESHOLD` 3→5 raise (design.md §4.4) — it's an unresolved open question, because a cumulative rate can hide a recent improvement, and the actual post-deploy cohort has never been isolated. The query is written but was run with the date placeholder unsubstituted and returned 0 rows — still owed:

  ```sql
  SELECT COUNT(*) AS dead_ok, SUM(first_seen = last_seen) AS never_swept
  FROM listings
  WHERE gone_at IS NOT NULL AND spec_status='ok' AND variation_id IS NULL
    AND gone_at >= strftime('%s','<3→5 deploy date>');
  ```

  Consequence either way: 77 listings whose lifespan was never measured is lost baseline signal, not just a bookkeeping curiosity. The original item's own caveat — "the math matching once is encouraging, not proof the mechanism is fully understood" — is looking well-placed. Resurrection cohorts (1082; 2836 ×7; 5670 ×3) are part of the same question — the 5670 cohort spanned two sellers, ruling out batch relisting.
- **`baseline_report.py::run_report()` does two full scans of the dead-listing pool where one would do (2026-09-20 live report finding).** It calls `derive_candidate_pool_stats(conn, ...)` and then `derive_candidates(conn, ...)` separately; `derive_candidates_with_stats()` (V0.13 Part B) already returns both from one `_derive()` call. Visible in the live run as two identical DEBUG negative-lifespan drop blocks. Pre-existing, not caused by the `profile_id` scoping fix below — not worth a milestone on its own, fix it the next time this file is open for another reason.
...
- **`gone_at` can precede a listing's final observation.** Root cause: a
  ...
  understatement gap.
- **`last_seen` can predate `first_seen` (V0.13 live-verification finding;
  delta −2 observed, 2 rows).** `record_sweep` stamps `last_seen =
  swept_at`, the sweep's START time, and a sweep takes minutes to run. A
  listing inserted by a 5-minute poll while a sweep is in flight, and
  included in that same sweep's seen-set, gets stamped with a timestamp
  earlier than its own insert — the row is then permanently ordered
  backwards. Nothing consumes it wrongly today: `_derive()`'s
  `first_seen != last_seen` and V0.9b's `CASE WHEN last_seen >
  first_seen` both happen to handle it correctly, but by accident of
  which operator they use, not by design. Two consequences worth
  knowing: `CandidatePoolStats.sweep_confirmed` counts these rows as
  confirmed (arguably right — a sweep did see it — but the lifespan
  derived from it is not a real measurement), and this is a SECOND,
  distinct mechanism behind negative lifespans, separate from the
  poll-observation-after-sweep case above and in `docs/learnings.md` L3
  (which L3 has been updated to cross-reference — it very plausibly
  explains that entry's previously-unexplained 291-second anomaly). Not
  fixed here. The real fix is stamping `last_seen` with the sweep's
  completion time rather than its start, which changes what `gone_at`
  means and needs its own milestone.
- **`test_concurrent_first_connect_against_a_fresh_file_does_not_crash_or_hang`
  ...
  correctness bug.
- **Sweep coverage measured at 96.3% (V1.0 live verification, 2026-09-18) — just above the 95% warning line, with a NULL-`lifespan_mins` listing turning up in the very first `explain_listing` sample checked by hand.** Not a V1.0 problem — the MCP server only reported what was already there. Same shape as the existing "Never-swept rate" item above (design.md §4.4/§4.7): a listing that was never confirmed present by a sweep. Follow-up, not yet run: the unmeasured-vs-total-gone ratio for this profile right now — if it's high, this is a sweep-pagination investigation (`sweep_page_limit`/`sweep_max_pages` against the current active-listing count), not an MCP concern.
- ~~**Baselines last recomputed ~6 days ago (V1.0 live verification, 2026-09-18) — alerts are currently scoring against stale percentiles.**~~ **Closed 2026-09-21.** The recompute is now automated — see the "Nightly baseline recompute shipped 2026-09-21" status entry above (design.md §17). The gap this item was flagging was real: the 2026-09-21 live run showed 741 dead candidates considered against 707 measured earlier that same evening, 34 new candidates accumulating in the hours since the previous manual run — which is exactly the kind of drift that used to go unmeasured for days at a time. `baseline_history` (design.md §15's Deliberately Deferred list) is a separate, still-open follow-up — it pairs naturally with this milestone but needs its own table and its own design pass.
- **`Ryzen PRO 8540U` pattern gap** — no digit between "Ryzen" and "PRO".
- Delete `reserve(n)`'s unused `n` parameter.
- **`scripts/recompute-baselines.sh`'s Kuma push is skipped silently when `DEALWATCH_KUMA_PUSH_URL` is unset (design.md §17, `docs/learnings.md` L19).** The push block is conditional on the variable being non-empty, so a missing/unset variable produces no error and no stderr line — the recompute itself still runs and reports `ok`, just with nothing sent to Kuma. Whether an unset variable should warn on stderr rather than pass quietly is undecided. Not fixed.
- **`scripts/recompute-baselines.sh` parses profile `id:`/`enabled:` with a line-prefix `awk` match** (`/^id:[[:space:]]/`), correct for the current flat top-level-key profile shape but silent if either key is ever indented or nested under something else — the match just returns empty, `id` fails the `-n` check, and the profile is skipped with a `FAIL ... no top-level id:` rather than read correctly. Worth revisiting if `profiles/*.yaml`'s shape ever changes.
