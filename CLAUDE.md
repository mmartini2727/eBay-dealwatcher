...
- **Never-swept rate: 22 of 217 dead listings (10%)**, consistent with the ~8% recorded pre-V0.8c — same order of magnitude, still unresolved, figure refreshed so the next comparison is against something current. Pagination doesn't explain it (V0.7c); cause unidentified. Do not tune `MISS_THRESHOLD` until it is. Resurrection cohorts (1082; 2836 ×7; 5670 ×3) are part of the same open question — the 5670 cohort spanned two sellers, ruling out batch relisting.
