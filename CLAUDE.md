...
- **V0.8d's pagination-drift estimate corrected (design.md §4.4, dated 2026-09-07).** The first real `sweeps` row measured a 0.48% duplicate rate, not the ~6% the `MISS_THRESHOLD` derivation assumed — the page-count estimate was inflated by container restarts firing extra sweeps, and (more importantly) the independent-miss model over-predicts by ~170× once given the correct rate. Misses are correlated, not independent — the same listings get missed repeatedly. `MISS_THRESHOLD=5` is still the right call (free given `gone_at = last_seen`) but its benefit is no longer a derived ~250×, it's empirical and TBD from the resurrection-rate trend.
- **V0.8e complete: wired `poll.sort` (design.md §4.5).** `profiles/thinkpad-t14.yaml`'s `poll.sort: newlyListed` had existed since the schema was written but `EbayBrowseProvider.search()` never sent it. `scripts/probe_sort.py` confirmed live that eBay honors it (a 50-item page's `itemCreationDate` spread collapsed from ~9 months to under 3 days) before any wiring happened. `search()` gained a keyword-only `sort` param; the fast poll passes `profile.search.poll.sort`, the sweep deliberately passes nothing. **This is a discovery-latency fix for V0.9 alerting, not a data-integrity fix** — the pagination-stability argument for it was retired by V0.8d's correction above. Deploy timestamp not yet recorded — see design.md §4.5's placeholder and the open item below.

### Open items

- **Record the V0.8e deploy timestamp the moment `poll.sort` wiring goes live** (design.md §4.5's placeholder). It's a lifespan-measurement discontinuity: catching listings ~28 minutes earlier on average inflates every post-fix lifespan relative to a pre-fix one for the same true lifespan — ~2% against `fast_lifespan_hours=24`, ~30% against a 90-minute sale. Any baseline recompute spanning this date mixes two measurement regimes.
...
- **Re-anchor the `2|intel-11th|16` seed override.** Its 230/250 was anchored on a stale computed p50 of $249; computed is now $215.00/$244.90 (n=24). design.md §5.6 deliberately deferred this until after the post-backfill recompute — that recompute has now run, so this is unblocked.
